...
//    		.filter(line -> line.split("\t").length > 6)
    		...
    		line -> {
    			...
    			Double likes = Double.parseDouble(lineArr[2]);
    			return new Tuple2<String, Double>(videoID, likes);
    		})
...
//			.take(10);
    ...
    List<Tuple2<Double, String>> topTenTuples = sortedRDD.take(20);
    JavaPairRDD<Double, String> topTenRdd = sc.parallelizePairs(topTenTuples);
    
    String output_dir = "output/spark/TopLikedVideos";
